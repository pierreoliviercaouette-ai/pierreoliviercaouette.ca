 ...
 * Piste haute dans le document + stage en portal fixed sur body
 * (styles de pin mis à jour en RAF, hors React, pour rester collé au scroll).
 */
export function AppleCinematicScroll({
  ...
  useEffect(() => {
    ...
    const onSeeked = () => {
      ...
      if (
        ...
        Math.abs(pending - video.currentTime) > 0.05
      ) {
  ...
  useEffect(() => {
    ...
    const syncPin = () => {
      ...
      // Hors viewport : masquer pour ne pas laisser un calque fantôme
      const offscreen =
        (mode === 'before' && rect.top >= vh) || (mode === 'after' && top <= -vh * 0.2);
      stage.style.visibility = offscreen ? 'hidden' : 'visible';
      stage.style.opacity = offscreen ? '0' : '1';
    };
    ...
    const tick = () => {
      ...
      if (Math.abs(smoothRef.current - lastUiRef.current) > 0.003) {
  ...
  const stage = (
    <div
      ref={stageRef}
      className="pointer-events-none overflow-hidden bg-dark"
      style={{
        position: 'fixed',
        left: 0,
        right: 0,
  ...
  return (
    <>
      {/* Piste invisible : seule source de hauteur / scroll */}
      <section
        ref={trackRef}
