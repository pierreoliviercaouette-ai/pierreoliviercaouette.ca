 ...
 * Sticky in-flow (pas de portal fixed) pour que le scroll tactile iOS reste libre.
 */
export function AppleCinematicScroll({
  ...
  useEffect(() => {
    ...
    const onSeeked = () => {
      ...
      const threshold = mobileRef.current ? 0.12 : 0.05;
      if (
        pending != null &&
  ...
  useEffect(() => {
    const applyVideo = (p) => {
      ...
      const minDelta = mobileRef.current ? 0.1 : 0.04;
    ...
    const tick = () => {
      ...
      // Sur mobile : suivre le scroll de plus près (moins de lag ressenti)
      const factor = reducedMotion ? 1 : mobileRef.current ? 0.55 : 0.3;
      const next = prev + (target - prev) * factor;
      smoothRef.current = Math.abs(target - next) < 0.0008 ? target : next;
      applyVideo(smoothRef.current);

      const uiStep = mobileRef.current ? 0.012 : 0.003;
  ...
  const stageFade = reducedMotion ? 1 : clamp01(1 - Math.max(0, p - 0.97) / 0.03);

  const mobileScrollVh = Math.min(scrollHeightVh, 480);
  ...
  return (
    ...
    >
      {/* Hauteur de piste : plus courte sur tactile pour un scrub jouable */}
      <style>{`
        [data-testid="apple-cinematic-scroll"] {
          height: ${reducedMotion ? 100 : scrollHeightVh}vh;
        }
        @media (hover: none), (pointer: coarse) {
          [data-testid="apple-cinematic-scroll"] {
            height: ${reducedMotion ? 100 : mobileScrollVh}vh;
          }
        }
      `}</style>
      ...
      <div
        className="sticky top-0 h-[100svh] w-full overflow-hidden bg-dark"
        style={{
          opacity: stageFade,
          // Laisse le doigt scroller la page ; seuls les CTA reprennent les clics
          pointerEvents: 'none',
          touchAction: 'pan-y',
        }}
        ...
        {outro && (
          <div
            ...
            style={{
              ...
              // Ne jamais bloquer le scroll : seuls les liens (pointer-events-auto) sont cliquables
              pointerEvents: 'none',
            }}
          >
